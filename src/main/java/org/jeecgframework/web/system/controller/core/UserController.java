...
//		List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			users.setPerson_type(user.getPerson_type());
			...
			if (users != null) {
				...
				user.setPerson_type(user.getPerson_type());
				// ------add by zhulei 2018/11/10
				user.setPersonLevel(Globals.USER_PERSON_TYPE_SYSTEM);
				// ------end-------
