...
//	}
	...
	@ResponseBody
	@Deprecated // add-begin-end--Author:zhangguoming Date:20140928
...
//			depart.setOrgCode(orgCode);
	...
	public void dataLogDatagrid(TSDatalogEntity datalogEntity, HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSDatalogEntity.class, dataGrid);
		// 查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, datalogEntity,
	...
	public AjaxJson ftpUploader(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if ("1".equals(upFlag)) {
				...
				if (uploadFtpFile(ftpUrl, Integer.valueOf(port), userName, passWord, path, fileName,
						mf.getInputStream())) {
					msg = "上传成功";
	 ...
	 */
	private boolean uploadFtpFile(String url, int port, String userName, String passWord, String path, String fileName,
			InputStream file) {
	...
	public AjaxJson filedeal(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if ("1".equals(upFlag)) {
				...
				logger.info("---fileDelete----" + fileDelete.getName());
