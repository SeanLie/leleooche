...
public class UserServiceImpl extends CommonServiceImpl implements UserService {
	...
	@Transactional(readOnly = true)
	public TSUser checkUserExitsByTel(String mobilePhone) {
		return this.commonDao.findUserByTel(mobilePhone);
	}

	/*
	 * @Transactional(readOnly = true) public TSUser checkUserCodeExits(String code)
	 * { return this.commonDao.findUserByCode(code); }
	 */
...
//		log.setTSUser(ResourceUtil.getSessionUser());
		...
		/*
		 * update-end--Author chenqian 201708031TASK #2317 【改造】系统日志表，增加两个字段，避免关联查询
		 * [操作人账号] [操作人名字]
		 */
		commonDao.save(log);
	}
	...
	public Map<String, TSFunction> getLoginUserFunction(String userId) {
		...
		StringBuilder hqlsb2 = new StringBuilder(
				"select distinct c from TSFunction c,TSRoleFunction rf,TSRoleOrg b,TSUserOrg a ").append(
						"where a.tsDepart.id=b.tsDepart.id and b.tsRole.id=rf.TSRole.id and rf.TSFunction.id=c.id and a.tsUser.id=?");
		...
		for (TSFunction function : primaryMenu) {
			if (function.getFunctionLevel() == 0) {
				...
				if ("业务申请".equals(lang_context)) {
					...
					floor += " <li style='position: relative;'>" + ss
					...
					floor += " <li style='position: relative;'>" + s
							+ "<img class='imag1' src='plug-in/login/images/msg.png' /> "
							+ " <img class='imag2' src='plug-in/login/images/msg_up.png' style='display: none;' /></li> ";
				} else {
					// 其他的为默认通用的图片模式
